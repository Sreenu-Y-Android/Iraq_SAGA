...
const Dashboard = () => {
  ...
  const getGrievanceCount = () => {
    ...
    if (grievancePlatform === 'all' && grievanceStatus === 'all') {
      ...
      if (karim && typeof karim.total === 'number') return karim.total;
    }
    return data.total ?? 0;
  };
              ...
              if (total === 0) {
                return (
                  <div className="h-full flex flex-col items-center justify-center p-8 text-center min-h-[400px]">
                    ...
                    <p className="text-sm font-medium text-slate-500">No grievance data available yet</p>
                    <p className="text-[11px] text-slate-400 mt-1">Sentiment analysis will appear here once grievances are ingested.</p>
                  </div>
                );
              }
